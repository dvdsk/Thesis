...
pub async fn update(state: &'_ State<'_>, dir: &Directory) {
    let geather_change_idx = state
        .chart
        .map
        .iter()
        .map(|m| m.value().clone())
    ...
    let cluster_idices: HashMap<ChangeIdx, usize> = futures::future::join_all(geather_change_idx).await
        .into_iter()
        .filter_map(|e| e)
...
pub async fn handle_update(state: &'_ State<'_>, dir: &mut Directory) {
